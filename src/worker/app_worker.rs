...
pub struct AppWorkerConfig {
    ...
    pub timeout: bool,
    /// The maximum duration workers should run for. The timeout is only enforced if `timeout` is
    /// `true` and `max-duration` is configured.
...
{
    /// Build a new instance of the [Self]
    ...
    ///
    /// The default implementation uses the value from the app's config file.
    fn max_duration(&self, state: &A::State) -> Option<Duration> {
        let context: Arc<AppContext> = state.clone().into();
...
mod tests {
    ...
    fn deserialize_config_override_max_duration() {
        ...
        .unwrap();
        assert_eq!(value.inner.max_duration, Some(max_duration));
    }
